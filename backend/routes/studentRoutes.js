...
// @desc    Update authenticated student's department (Disabled - Read-only)
// @access  Private (Student)
router.patch('/profile/department', protect, async (req, res) => {
  return res.status(403).json({
    success: false,
    message: 'Department is read-only and cannot be modified from the student profile.'
  });
});

// @route   PATCH /api/students/profile/year
// @desc    Update authenticated student's academic year (Disabled - Read-only)
// @access  Private (Student)
router.patch('/profile/year', protect, async (req, res) => {
  return res.status(403).json({
    success: false,
    message: 'Year is read-only and cannot be modified from the student profile.'
  });
});
